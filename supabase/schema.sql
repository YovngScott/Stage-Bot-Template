...
--                                        equipo lo atienda. La IA puede
--                                        levantar esta bandera al escalar un
--                                        caso SIN silenciar al bot.
...
-- ----------------------------------------------------------------------------
-- 5e. ASISTENTE VIRTUAL — triaje de correo (bots con kind = 'assistant').
--
--    MINIMIZACIÓN DE DATOS (deliberada): NO se guarda el cuerpo del correo.
--    Una vez clasificado el mensaje y creado el borrador, el contenido se
--    descarta de la memoria del proceso y aquí solo persisten metadatos,
--    la clasificación y los identificadores necesarios para auditar. Esto
--    mantiene el correo corporativo del cliente fuera de nuestra base.
-- ----------------------------------------------------------------------------
create table if not exists asistente_correos (
  ...
  -- Resultado del filtro heurístico: si se descartó, aquí queda el motivo y
  -- la clasificación viene toda en null (nunca llegó a la IA).
...
-- El dashboard del ejecutivo lee su propio triaje (solo lectura: quien
-- escribe es el backend con la service_role key).
